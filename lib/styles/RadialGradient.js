...
export class RadialGradient extends Gradient {
  computeStyle(renderingLayer, boundingBox) {
    ...
    const centerPoint = this.start.clone().multiple(boundingBox.size).add(boundingBox.origin).absolute();
    const radiusPoint = this.end.clone().multiple(boundingBox.size).add(boundingBox.origin).absolute();
