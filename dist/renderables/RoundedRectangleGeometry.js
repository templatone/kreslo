...
export class RoundedRectangleGeometry extends Geometry {
  constructor(width, height, ...radius) {
    const d = (ctx, pxs, t) => {
      ...
      if (r1.x > 0 && r1.y > 0) {
        r1.multiple(Numbers.limit(r1.x, 0, width2 / 2) / r1.x);
        r1.multiple(Numbers.limit(r1.y, 0, height2 / 2) / r1.y);
      }
      const r2 = this.topRightRadius.clone();
      if (r2.x > 0 && r2.y > 0) {
        r2.multiple(Numbers.limit(r2.x, 0, width2 / 2) / r2.x);
        r2.multiple(Numbers.limit(r2.y, 0, height2 / 2) / r2.y);
      }
      const r3 = this.bottomRightRadius.clone();
      if (r3.x > 0 && r3.y > 0) {
        r3.multiple(Numbers.limit(r3.x, 0, width2 / 2) / r3.x);
        r3.multiple(Numbers.limit(r3.y, 0, height2 / 2) / r3.y);
      }
      const r4 = this.bottomLeftRadius.clone();
      if (r4.x > 0 && r4.y > 0) {
        r4.multiple(Numbers.limit(r4.x, 0, width2 / 2) / r4.x);
        r4.multiple(Numbers.limit(r4.y, 0, height2 / 2) / r4.y);
      }
      ...
      ctx.ellipse((width2 - r2.x) * pxs, r2.y * pxs, r2.y * pxs, r2.x * pxs, Math.PI * 1.5, 0, Math.PI / 2);
      ctx.ellipse((width2 - r3.x) * pxs, (height2 - r3.y) * pxs, r3.x * pxs, r3.y * pxs, Math.PI * 2, 0, Math.PI / 2);
      ctx.ellipse(r4.x * pxs, (height2 - r4.y) * pxs, r4.y * pxs, r4.x * pxs, Math.PI * 0.5, 0, Math.PI / 2);
      ctx.closePath();
    };
    const b = (t) => {
      return {
